...
pub const InputBuffer = struct {
    tokens: [16][128]u8 = [_][128]u8{[_]u8{0} ** 128} ** 16,
    token_counter: u8 = 0,
    char_counter: u8 = 0,

    pub fn push(self: *InputBuffer, val: u8) void {
        if (val == ' ') self.token_counter += 1;
...
pub const Shell = struct {
    ...
    pub fn init(name: [:0]const u8) Shell {
        return Shell {
            .name = name,
            .window = owos.ui.window.Window.init(name, owos.c.SCREEN_WIDTH / 2 - 400, owos.c.SCREEN_HEIGHT / 2 - 250, 800, 500),
            .cursor = Cursor.init(),
            .buffer = InputBuffer{},
        };
    }
    ...
    pub fn tick(self: *Shell) anyerror!u8 {
        ...
        if (self.grid_dirty or !self.cursor.was_drawn) {
    ...
    pub fn greet(self: *Shell) void {
        ...
        self.cursor.newline();
        self.print("Developer: ", 0x7777FF);
        self.println("Elias Stettmayer", 0x77FF77);
        self.print("Repository: ", 0x7777FF);
        self.println("www.github.com/lordaftereight/owos", 0x77FF77);
    ...
    pub fn handle_command(self: *Shell) u8 {
        if (owos.c.strcmp(@ptrCast(&self.buffer.tokens[0]), "exit\x00")) {
            ...
            while (true) {
                asm volatile ("hlt");
                const c = owos.c.ps2_poll();
                if (c != 0) {
                    if (c == 'y') {
                        self.print("y", 0xFFFFFF);
                        self.cursor.newline();
            ...
            if (owos.c.strcmp(@ptrCast(&self.buffer.tokens[1]), "list\x00")) {
                for (owos.scheduler.global_scheduler.processes) |process| {
                    if (process != null) {
                        self.print("Process: ", 0x7777FF);
                        self.print(process.?.name, 0x7777FF);
                        self.print_at(25, "PID:", 0x7777FF);
                        var buf = [_:0]u8{0} ** 4;
                        owos.c.format(@ptrCast(&buf), "%d", process.?.id);
                        self.print_at(29, &buf, 0x7777FF);
                        if (process.?.running == true) {
                            self.println_at(32, "Running", 0x7777FF);
                        } else {
                            self.println_at(32, "Halted", 0x7777FF);
                        }
                    }
                }
            } else if (owos.c.strcmp(@ptrCast(&self.buffer.tokens[1]), "kill\x00")) {
                self.print("Are you sure? (y/n) ", 0xFFFFFF);
                while (true) {
                    asm volatile ("hlt");
                    const c = owos.c.ps2_poll();
                    if (c != 0) {
                ...
                self.println("Invalid proc command", 0xFF7777);
            }
            ...
            var buf = [_]u8{0} ** 32;
            self.print("Invalid command: ", 0xFF7777);
    ...
    pub fn handle_input(self: *Shell) void {
        ...
        switch (input) {
            ...
            10 => {
                ...
                self.buffer.tokens = [_][128]u8{[_]u8{0} ** 128} ** 16;
            },
            0 => {},
            else => {
