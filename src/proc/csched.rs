...
impl CooperativeScheduler {
    ...
    pub fn add_process<T: crate::proc::Process + 'static>(&mut self) {
        ...
        crate::proc::registry::PROCESS_TABLE.lock().push(crate::proc::registry::ProcTableEntry {
            pid: process.pid(),
            name: process.name(),
            ...
            match cmd {
                ...
                SchedulerTask::Kill(pid) => {
                    if let Some(idx) = self.procs.iter().position(|p| p.pid() == pid) {
                        ...
                        crate::proc::registry::PROCESS_TABLE.lock().retain(|e| e.pid != pid);
                        proc.on_uninit();
                    }
                }
            }
    ...
    pub fn start(&mut self) -> Result<(), SchedulerError<crate::proc::ProcessError>> {
        loop {
            ...
            while i < self.procs.len() {
                ...
                loop {
                    match self.procs[i].on_tick() {
                        Err(err) => {
                            ...
                            crate::proc::registry::PROCESS_TABLE.lock().retain(|e| e.pid != proc.pid());
                            proc.on_uninit();
                ...
                if let Some(entry) = crate::proc::registry::PROCESS_TABLE.lock().iter_mut().find(|e| e.pid == pid) {
                    entry.status = status;
                }
                i += 1;
            }
